...
export default function ColorAboutPage() {
    const params = useParams();
    const hexParam = params?.hex as string;

    const hex = useMemo(() => {
        if (!hexParam) return '#000000';
        return hexParam.startsWith('%23') ? `#${hexParam.replace('%23', '')}` : `#${hexParam}`;
    }, [hexParam]);

    const [mounted, setMounted] = useState(false);
    const [dbColor, setDbColor] = useState<{ name: string, hex: string, description?: string, psychology?: string, meaning?: string, usage_info?: string } | null>(null);
